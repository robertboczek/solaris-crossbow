	...
	public SelectFlowForChart(Shell parentShell, ObjectModel objectModel,
		...
		addButton.addListener(SWT.MouseUp, new Listener() {
			...
			public void handleEvent(Event arg0) {
				...
				if (list2.getItems().length + selectedTab.length > 3) {
					MessageDialog
							.openError(null, "Error",
									"You can display maximally 3 flows on the same chart");
					return;
				}
				...
				if (list2.getItems().length > 0
						&& chartTimeType.getSelectionIndex() >= 0) {
		...
		removeButton.addListener(SWT.MouseUp, new Listener() {
			...
			public void handleEvent(Event arg0) {
				...
				removeButton.setEnabled(false);
				if (list2.getItems().length == 0
						|| chartTimeType.getSelectionIndex() == -1) {
		...
		chartTimeType.setData("Last minute", LinkStatisticTimePeriod.MINUTELY);
		chartTimeType.add("Last 5 minutes");
		chartTimeType.setData("Last 5 minutes",
				LinkStatisticTimePeriod.FIVE_MINUTELY);
		chartTimeType.add("Last hour");
		chartTimeType.setData("Last hour", LinkStatisticTimePeriod.HOURLY);
		chartTimeType.add("Last day");
		chartTimeType.setData("Last day", LinkStatisticTimePeriod.DAILY);
