...
public class FlowDialog extends TitleAreaDialog{
	...
	protected Control createDialogArea(Composite parent) {
		...
		flowName.addFocusListener(new FocusListener(){
			...
			public void focusLost(FocusEvent arg0) {
				if(!flowName.getText().equals("")){
					...
					setMessage("Flow name can't be empty", IMessageProvider.ERROR);
				}
			}			
		});
		
		flowName.setText(policy.getName());
		...
		if(policy.getFilter() instanceof PortFilter) {
			...
			protocol.setData("tcp", org.jims.modules.crossbow.objectmodel.filters.TransportFilter.Protocol.TCP);
			protocol.setData("udp", org.jims.modules.crossbow.objectmodel.filters.TransportFilter.Protocol.UDP);
			protocol.setData("sctp", org.jims.modules.crossbow.objectmodel.filters.TransportFilter.Protocol.SCTP);
			
			int i = 0;
			for(i=0; i<protocol.getItems().length; i++) {
				if(protocol.getData(protocol.getItem(i)).equals(transportFilter.getProtocol())) {
					protocol.select(i);
					break;
				}
			}
	...
	private void saveInput() {	
		...
		policy.setName(flowName.getText());
		...
		if(policy.getFilter() instanceof PortFilter) {
			...
			transportFilter.setProtocol((org.jims.modules.crossbow.objectmodel.filters.TransportFilter.Protocol)protocol.getData(protocol.getText()));
