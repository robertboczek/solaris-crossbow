  ...
  \chapter{Context}  % TODO ładniej
    ...
    \section{Multilevel network virtualization}
      ...
      \subsection{Definition} 
        ...
        Network virtualization is based on providing seperate networking environment for every defined groups ( existing within 
        environment ). For each group logical environments are created over single existing physical network infrastructure. 
        These created networks provide complete network services so that from the end-user of one of existing groups perspective 
        dedicated network with policies and dedicted resources are assigned. Network virtualization concerns not only logical 
        segmentation of the network transport, but also network devices and all other network services. \cite{network_virt}
