...
\begin{document}
  ...
  \chapter{The system architecture}
    ...
    % \section{The environment}  TODO tutaj? wymagania? kontekst?

    % TODO kilka widokow:  jmx (manager+entity), iaas (nad jmx), statystyki
    % TODO osobny opis do jmx!


    % OUR SYSTEM                     +<-- model            <--+
    %                                v    instantiate         ^  discover
    % HOST SYSTEM


    \section{High-level design}

      % TODO opis srodowiska - ,,soft'' + sprzet pod spodem i ich cel

      The specific character of the system --- running in a distributed environment, moderate complexity --- requires
      proper architectural model. The model should allow to design the elements of the system to be highly cohesive and
      maintain coupling as loose as possible. Each component has its own well-defined role and exposes a set of
      operations to interact with others.

      % TODO dodac: system rozproszony, latwe skalowanie


      \subsection{Main use cases}  % TODO change!
      ...
      \subsection{Important stages}  % TODO change!

        There are three main stages identified when working with the topologies. There is a purely logical stage that
        does not require any knowledge of the underlying environment --- the operations involve manipulating the domain
        model to create or update the virtual topology. There is a assignment stage which results in association between
        the model and physical resources. Finally, the actual deployment takes place in model instantiation stage ---
    ...
    \section{System components and their responsibilities}
      ...
      \subsection{Virtual Appliance Repository}

        The main responsibility of Virtual Appliance repository is to maintain the list and provide access to virtual
        appliances created. It is used in two phases: the design phase to select appliances that are to be deployed, and
        instantiation phase to serve virtual appliance images.
      ...
      \subsection{Supervisor}
        ...
        Supervisor delegates most of the work to worker nodes. Transactional operations can be provided at this level by
        extending the supervisor's behaviour to rollback after one of the worker nodes fails.

        % TODO seq. diagram

        \noindent
        \begin{minipage}{\textwidth}
          \lstinputlisting[caption={Supervisor component public interface},label={lst:arch:iface:supervisor},language=Java]{lst/arch-iface-supervisor.java}
        \end{minipage}
      ...
      \subsection{Worker}
        ...
        \noindent
        \begin{minipage}{\textwidth}
          \lstinputlisting[caption={Worker component public interface},label={lst:arch:iface:worker},language=Java]{lst/arch-iface-worker.java}
        \end{minipage}
      ...
      \begin{figure}[H]
        \begin{center}
          \begin{tikzpicture}[start chain,
            ...
            % TODO node distance + alignment

            \node (jmx)  [minimum width=9cm]             {Java Management Extensions};
            \node (midl) [below right=of jmx.south west] {shell scripts};  \node (midr) [right=of midl] {native libraries};
    ...
    \section{Domain model and data flows} \label{sec:domain-model}
      ...
      Created object model allows describing network structure contaning of: switches, resources with attached
      addressable interfaces and flows with policies. Figure below presents concrete classes and their dependencies. 
