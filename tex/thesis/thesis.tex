...
\begin{document}
  ...
  \label{chap:intro}
    ...
    In today's world every successful organisation is based on properly designed communication networks. These networks
    ...
    by controlling delay, delay variation (jitter), bandwidth and packet loss parameters is deeply hidden secret of most
    successful end-to-end business applications.
    
    Technologies like Ethernet did not foresee \todo[inline]{raczej nie byly zaprojektowane; chyba, ze damy zrodlo} future
    ...
    in field of resource reservation and isolation. 

    % http://www.cisco.com/en/US/products/ps6558/products\_ios\_technology\_home.html

    Increasing popularity of virtualization, isolation and resource reservation in contemporary systems encourages
    further research.
    ...
    \section{Thesis objective}
      ...
      component-based architecture which enables construction of a system that would facilitate working with fully
      isolated virtualized network and resources grouped by project name.}

      Due to substantial lack in similar products available on market research in this area was planned. The main system
      ...
      mechanism contained in the Crossbow project, Solaris OS seems to be suitable environment to proceed further
      research.
    ...
    \section{Organization of the thesis}
      ...
      Chapter \ref{chap:bck} provides background information about network and resource virtualization, \gls{acr:qos}
      together with description of existing solutions in these areas. Chapter \ref{chap:req} is a chapter concerning
      about essential functionalities accompanied with non-functional requirements. Subsequent chapter \ref{chap:sol}
      presents Solaris OS features with special attention paid to its new functionality: \textit{The Crossbow}. Chapters
      \ref{chap:arch} \todo[inline]{rozdzielic} and \ref{chap:impl} introduce \gls{acr:cm4j} system architectural and
      implementation details, respectively. Part of thesis in which implemented system work and usage is described is
      \ref{chap:cs}. In order to present all implemented aspects and functionalities of \gls{acr:cm4j} system a scenario
      with multimedia streaming server, \gls{acr:vod} and end user differentiation was prepared. Last chapter
      \ref{chap:sum} summarizes research performed in the field of virtualization. In that section not only achieved
      goals are described, but also unachieved objectives are highlighted. Possible improvements and new functionalities
      that might be added are also mentioned in this part. 

      \todo[inline]{wiecej o kazdym rozdziale}
  ...
  \label{chap:req}
    ...
    \todo[inline]{przeredagowac ten akapit - bez odniesien do innych rozdzialow}
    ...
    \section{Domain specification}
      ...
      \todo[inline]{definicje (abstrakcyjne): topology, instantiation, discovery, monitoring, host machine, underlying environment}

      \todo[inline]{obazrek: instantiation, discovery, monitoring}

      \todo[inline]{podejscie ogolne i ograniczenia, jakie my przyjmujemy}
    ...
    \label{sec:req:func}
      ...
      division is based on the quality of the functionality \todo[inline]{co to znaczy?}. 
      ...
      \subsection{Network design and adjustment}

        \todo[inline]{tutaj o skladaniu sieci w gui - wiekszosc z tego, co jest w Instantiation}
      ...
      \label{sec:req:func:inst}
        ...
        \begin{itemize}
          \item Creation of any requested virtual network element \todo[inline]{jakie network elements obslugiwane?},
          \item Creation of any requested resource \todo[inline]{czym jest resource? definicja} from repository,
          \item Assignation any number of links to resource,
          \item Resource routing table modification,
          \item Defining \gls{acr:qos} limits for links and flows \todo[inline]{zdefiniowac flow} (limits traffic assigned to specific network traffic type),
    ...
    \label{sec:req:nonfunc}
      ...
      \begin{itemize}
        ...
        \item adaptable --- ability to adapt to changing conditions (adding removing new nodes \todo[inline]{jakie node-y?}),
        \item fault tolerance --- system should be resistant to minor errors \todo[inline]{bardziej szczegolowo}.
      \end{itemize}
      ...
      \begin{itemize}
        ...
        \item extensibility --- ability to add new earlier unspecified functionalities \todo[inline]{open/closed principle in
                                design},
        \item composability --- system should be created in form of composable components \todo[inline]{coupling, cohesion},
        \item reusability --- ability to (re)use in future systems.
      \end{itemize}

      \todo[inline]{multiple APIs depending on abstraction level required}
      \todo[inline]{embeddable - JIMS integration}
  ...
  \label{chap:impl}
    ...
    \todo[inline]{integracja z JIMSem jest za wczesnie - wg mnie powinno byc przed albo w Crossbow infrastructure project}
    ...
    Section \ref{sec:impl:env} informs about \todo[inline]{inne wyrazenie} implementation environment aspects like requested operating system,
    ...
    \label{sec:impl:env}
      ...
      \todo[inline]{crossbow-mbean: Java, JMX, C}
      \todo[inline]{crossbow-gui: Java, SWT, Swing}
      \todo[inline]{wykopac JIMSa dalej}
      ...
      \begin{itemize}
        \item Notifications \todo[inline]{rozwinac, co to},
        \item MLet (downloading dynamic modules) \todo[inline]{rozwinac}.
      \end{itemize}
      ...
      \todo[inline]{ponizsze przynajmniej w osobnej podsekcji albo sekcji}
      ...
      \label{sec:impl:comp}
        ...
        As already mentioned each entities operation's are adjusted to their needs. Three figures below \todo[inline]{reference
        them, describe} depict in detail each manager and its corresponding entity.
        ...
        \todo[inline]{strzalki na \ref{fig:impl:jc-components} w dobra strone?}
      ...
      \label{sec:impl:low}
        ...
        \gls{acr:jims} layered architecture implicated \todo[inline]{to nie wynikalo z designu JIMSa} the demand for an approach towards accessing low-level functions.
        ...
        some cases \todo[inline]{np. kiedy?} accessing native libraries through libraries like \gls{acr:jna} or \gls{acr:jni} gives more
      ...
      \label{sec:impl:infrastructure}
        ...
        The whole system is based on \gls{acr:jmx} technology and externs \todo[inline]{?} existing \gls{acr:jims}
      ...
      \label{sec:impl:persist}
        ...
        Although created system does not provide data persistence in databases or any other formats \todo[inline]{nasza
        persystencja to jest jakis tam format} persistence is
      ...
      \label{sec:impl:verif}
        ...
        up to integration tests testing creating whole network structure. \todo[inline]{rozszerzyc}
      
        Additionally verification on the \gls{acr:gui} side of the system was provided where network to be created is
        verified in respect of proper names, \gls{acr:ip} addresses, etc. \todo[inline]{ten akapit do sekcji o GUI}
      ...
      \label{sec:impl:gui}
        ...
        \begin{itemize}
          \item Connecting to \gls{acr:jims} Gateway \todo[inline]{czy mamy to zdefiniowane?},
    ...
    environment. \todo[inline]{do summary} 


    \section*{Summary}
    \label{sec:impl:summary}

      This chapter introduced chosen approach to system implementation. Despite the fact that requested functionalities
      were demanding and complex, final product meets all previously listed functional requirements. Thanks to used
      technologies such as \gls{acr:jmx}, system with fully independent business layer (loosely coupled MBeans) was
      implemented. These technologies also helped in fulfilling defined non-functional requirements like: evolvability,
      extensibility, composability, etc. Definitely one of the biggest advantage of this system (apart from all
      delivered functionality provided by \gls{acr:cm4j}) is reusability as four possible usage models exist. System may
      be managed from implemented \gls{acr:gui}, running JConsole or even from own developed (Java-based or native)
    ...
    \label{sec:uc:operation}
      ...
      \label{sub:uc:limit}
        ...
        \begin{figure}[H]
          ...
          \begin{tikzpicture}[node distance=0]
            \node (img) at (0,0) {\includegraphics[width=.7\textwidth]{img/test-case/limit.png}};
      ...
      \label{sub:uc:traffic}
        ...
        \begin{figure}[H]
          ...
          \begin{tikzpicture}[node distance=0]
            \node (img) at (0,0) {\includegraphics[width=.7\textwidth]{img/test-case/vod-rtp.png}};
            \node [below=of img.south west] {time $[\mathrm{s}]$};
      ...
      \label{sub:uc:client}
        ...
        \begin{figure}[H]
          ...
          \begin{tikzpicture}[node distance=0]
            \node (img) at (0,0) {\includegraphics[width=.7\textwidth]{img/test-case/exp-all.png}};
            \node [below=of img.south west] {time $[\mathrm{s}]$};
  ...
  \label{chap:sum}
	  ...
	  Section \ref{sub:sum:concl} briefly concludes research results and test-case results. Thesis statement's legitimacy and feasibility 
		is also discussed in this section. Accomplished goals and objectives are outlined in section \ref{sub:sum:achieved} yet
	  possible improvements that might be introduced to created system are considered in section \ref{sub:sum:further}.

    \section{Conclusions}
		\label{sub:sum:concl}
		
      Proposed at the beginning of the thesis statement claiming that: \emph{There exists a layered architecture, which
      enables the construction of a system, that would facilitate working with fully  isolated virtualized network and
      resources grouped by project name} was deeply considered throughout this thesis by the authors.  In order to
      fulfil proposed statement detailed requirement analysis was performed. Subsequently in order to comply with all
      defined requirements mentioned in the thesis statement layered architectural approach was chosen. This approach
      allowed to distinguish certain groups of independent, highly cohesive components. Together these components create
      the planned system, but each one could be used separately allowing performing operations like: \gls{acr:crud}
      operations over virtual network elements, virtual resources management, etc. 
      
      Investigation performed while working on this paper allowed the authors to understand concepts of complex network
      virtualization and resource reservation and sharing in more details. Conducted tests additionally outlined the
      significance of these issues and how important they are in terms of success of proper work of complex systems.
      Encountered problems only confirmed that although very useful they are also demanding and require comprehensive
    ...
    \section{Achieved goals}
      ...
      In relation to achieved goals not all were unfortunately successfully finished. Time shortage and lack of people
      did not allow for as deep insight look at this issue as was initially planned. However, some objectives are really
    ...
    \section{Further work}
      ...
      component initially planned was automatic resource assigner, that would run and perform automatic resource
      assignments to nodes that run under lowest load. This assigner with attached rule-based system would gather data
      about the load on each node and based on that decide what and where to instantiate.  Presented and discussed
      system in this thesis lacks that functionality. Instead, it offers manual assignments, where user selects on which
      node new virtual resources should be created. Another not implemented yet welcomed would be semi-automatic
      snapshot creation. Currently user can create Solaris zone from existing snapshot and use it.  Functionality where
      created resource could be converted to snapshot and then transferred to the repository would definitely improve
