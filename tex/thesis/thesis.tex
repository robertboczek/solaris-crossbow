  ...
  \label{chap:bck}
    ...
    \label{sec:ctx:virt}
      ...
      \subsection{OS-level virtualization}
        ...
        systems must be the same, although different libraries and distributions are allowed\todo[inline]{uproscic to
        zlozone zdanie}. OS-level virtualization may be often presented as slicing single OS to small partitions
        (sometimes called virtual environments (VE), virtual private servers (VPS) or zones). From the end-user
        perspective these virtualization is invisible (user receives guest OS as real server).
