  ...
  \chapter{Context}  % TODO ładniej
    ...
    \begin{enumerate}
      ...
      \item differentiated services - uses TOS field (IPv4) or Traffic Class field (IPv6) and can work
            on packet by packet basis and does not need detailed information about flows
            % TODO ^  bardziej ogolnie? (nie wymaga rezerwacji na sciezce itd)
    \end{enumerate}
    ...
    \section{Multimedia server}
      ...
      \label{sub:case:prep}
        ...
        \label{ssub:case:prep:va}
          ...
          % zfs snapshot -r <path>@SNAP
          % zfs send -R <path>@SNAP > /appliance/<appname>.SNAP
          %
          % Example:
          % zfs snapshot -r rpool/Zones/darwin@SNAP
          % zfs send rpool/Zones/darwin@SNAP > /appliance/darwin.SNAP
