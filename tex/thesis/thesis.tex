...
\begin{document}
  ...
  \chapter{Introduction}
   ...
   \section{Thesis statement}
    ...
    \section{Organisation of the thesis}
      ...
      system CM4J architectural and implementation details. %todo cos madrego dopisac
  ...
  \label{chap:req}
    ...
    system and the work products (created during system development). Executing system are related to user goals and are
    referred as run-time qualities, whereas work products are driven by the development organization’s goals and
    ...
    Network isolation is provided thanks to the Crossbow functionality which is more accurately described in the chapter
                  ...
                  \begin{figure}[H]
                    \centering
                      ...
                      \begin{tikzpicture}
                        ...
                        \umlclass[x=-3, y=-3]{PriorityPolicy}
                        {
                          + priority : LOW / MEDIUM / HIGH
                        }
                        {}

                        \umlclass[x=3, y=-3]{BandwidthPolicy}
                        {
                          + limit : int
                        }
                        {}

                        \umlemptyclass[x=5, y=1]{Filter}

                        \umlemptyclass[x=8]{AnyFilter}

                        \umlclass[x=8, y=-3]{PortFilter}
                        {
                          + protocol : String \\
                          + port : int \\
                        ...
                        \umlclass[x=8, y=-6]{TransportFilter}
                        {
                          + protocol : String
                        }
                        {}

                        \umlclass[x=8, y=-9]{IpFilter}
                        {
                          + address : String \\
                          + netmask : int \\
                          + location : LOCAL / REMOTE
                        ...
                        \umlinherit{PriorityPolicy}{Policy}
                        \umlinherit{BandwidthPolicy}{Policy}
  ...
  \label{chap:impl}
        ...
        \begin{table}[H]
          ...
          \begin{tabular}{llp{4cm}}
            ...
            WorkerMBean               & \textbf{Crossbow:type=XBowWorker}           & Responsible for instantiating on
                                                                                      \newline this node given model
                                                                                      with regard to specified  actions  \\
            \hline
            RepoManagerMBean          & \textbf{Crossbow:type=RepoManager}          & Allows getting/setting path to
                                                                                      projects placement, \newline
                                                                                      returns all existing projects from
                                                                                      working path                       \\
            \hline
            ...
            VNicManagerMBean          & \textbf{Crossbow:type=VNicManager}          & Provides \gls{acr:crud} operations
                                                                                      for \gls{acr:vnic}s                \\
            \hline
            VlanManagerMBean          & \textbf{Crossbow:type=VlanManager}          & TODO co to robi?                   \\
            \hline
    ...
    \label{sec:impl:summary}
      ...
      This chapter introduced to adopted approach to system implementation. Despite the fact that requested
      ...
      (loosely coupled MBeans) were implemented. These technologies also helped in fulfilling defined non-functional
  ...
  \label{chap:cs}
    ...
    \label{sec:uc:prep}
      ...
      are used as building blocks when designing the network. Finally, the virtual infrastructure is instantiated, i.e.
      all the underlying low-level components, like zones, etherstubs, \gls{acr:vnic}s and flows, are created.
  ...
  \label{chap:sum}
	  ...
	  Section \ref{sub:sum:concl} briefly concludes research results and test-case results. Thesis's statement legitimacy and feasibility 
		is also discussed in this section. Realized goals and objectives are outlined in the section \ref{sub:sum:achieved} yet
	  possible improvements that might be introduced to created system are considered in the \ref{sub:sum:further} part.

    \section{Conclusions}
		\label{sub:sum:concl}
		
		Proposed at the beginning of the thesis statement claming that: 
		\emph{There exists a layered architecture, which enables the construction of a system, that would facilitate working with 
		fully  isolated virtualized network and resources grouped by project name } was deeply considered throughout this thesis by 
		the authors. 
		In order to fulfil proposed statement detailed requirement analysys were performed. Subsequently in order to comply
		all definied requirements mentioned in thesis statement layered architectural approach accepted. This approach allowed 
		to distinguish certain group of independent, highly coupled components. Together these components provided planned
		system, but each one could be used separately allowing performing operations like: CRUD operations over virtual network elements, 
		virtual resources management, etc. 
		
		Investigation performed during work on this paper allowed the authors to understand concepts like network virtualization,
		resource reservation in more detailed. Conducted tests additionally outlined the significance of these issues and how important they
		are in terms of success of proper work of complex systems. Encountered complex problems only confirmed that although very useful 
    ...
    \section{Achieved goals}
		...
		\begin{itemize}
			\item Significant improvement in system configuration in comparison to manual setup,
			\item Automatic update of the given network parameters,
			\item Reusability provided thanks to coupled components hidden under well definied interfaces,
	...
	Probably the largest component,  which was initially plannned, was automatic resource assigner, that would run and 
	perform automatic resource assignments to nodes that run under lowest load. This assigner with attached rule based
	system would gather data about the load on each node and based on that decide what and where instantiate. 
	...
	be semi-automatic snapshot creation. Currently user can create solaris zone from existing snaphot and use it. 
	Functionality where created resource could be converted to snapshot and then transfered to the repository would 
	definately improve the usability of the system.
