...
\begin{document}
  ...
  \chapter{Case Study}
    ...
    infrastructure are listed and described. A number of tests were performed to evaluate the created system. Resulting
    experimental data is presented and discussed.

    Section \ref{sec:uc:description} introduces the overall view of the topology that was created. Main components are
    described and QoS requirements are discussed.

    Section \ref{sec:uc:diff} discusses Quality of Service requirements in more detail. Types of service are presented
    and appropriate network-level policies described. The policies are assigned to network components.

    Section \ref{sec:uc:prep} describes the steps needed to set up the topology. The steps include virtual appliance
    ...
    system. The section focuses on Quality of Service-related aspects --- it verifies definition, management and
    ...
    \label{sec:uc:description}
      ...
      far as audio and video streaming is concerned, data must be supplied to the end user with special focus on
      minimizing the delay and jitter.

      The types of multimedia services map directly to QoS policies that are required. Table \ref{tab:uc:qos} shows the
      mapping.
      ...
      favoured in order to provide desired level of quality --- precedence when accessing transport media and low-jitter
      transfer. This allows for low-delay streaming with small input data buffers.
      % TODO < zmienic? zrodlo?
      ...
      medium-priority or even best-effort data (no CIR).  % TODO < citation?
      ...
      an HTTP server handling VOD requests. The clients are able to connect to the streaming server and start streaming
      ...
      specify fine-grained policies for clients in subnetworks regardless of the ones defined at the server level.
      ...
      whole design remains clear and maintainable.
      
      Figure \ref{fig:cs:scenario} presents an overview of the network structure and client configuration. No details
      are depicted, only basic structure.
    ...
    \label{sec:uc:prep}
      ...
      \label{ssub:case:prep:va}
        ...
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={New zone creation}]
        ...
        \noindent
        It may be necessary to modify \texttt{/etc/shadow} file to be able to access the zone with zlogin. \\

        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={\texttt{/etc/shadow} file adjustment}]
            # sed s/root::/root:NP:/ /etc/shadow
          \end{lstlisting}
        \end{minipage}

        After installation the zone can be booted and used after logging. \\

        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={Booting and logging into a zone}]
            # zoneadm -z mplayer boot
            # zlogin mplayer
          \end{lstlisting}
        \end{minipage}

        \noindent All the required software should be installed now. When the zone is prepared, a ZFS snapshot can be
        taken and transferred to the repository. \\

        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={Taking a zone snapshot}]
        ...
        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={Publishing a snapshot in NFS repository}]
      ...
      \label{ssub:}
        ...
        The topology consists of router appliance (able to forward IP packets between its directly-attached interfaces),
        server appliance (with Darwin Streaming Server and thttpd HTTP server installed) and client appliance (mplayer
        with RTP streaming support enabled). All the appliances are instantiated on a single physical host.
      ...
      \label{sub:uc:xbow}
        ...
        There are five zones created (one media server, three clients and one router). \\

        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={All the zones created after model instantiation}]
        ...
        server zone and all of the client zones are connected to the router zone with etherstubs. \\

        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={Etherstubs}]
        ...
        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={Virtual interfaces and flows created on top of them}]
      ...
      \label{sub:}
        ...
        \noindent
        \begin{minipage}{\textwidth}
          \begin{lstlisting}[caption={Media preparation before streaming}]
    ...
    \label{sec:uc:enhance}
      ...
      The implemented system provides extensive support for all the stages that comprise virtual network management.
      ...
      \label{sub:uc:enhance:design}
        ...
        The GUI console displays the topology in a form of a graph --- with virtual appliances (or switches) as nodes
        ...
        way to set up addressing, routing and quality policies. Also, NFS repository access is integrated. Input data
        describing the model is validated while being entered by the user. 
      ...
      \label{sub:uc:enhance:online}
        ...
        policies, for example, already instantiated can be changed, when needed.
