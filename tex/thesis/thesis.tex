...
\begin{document}
  ...
  \chapter{Case Study}
    ...
    \label{sec:uc:description}
      ...
      in multimedia-oriented networks. As far as audio and video streaming is concerned, it must be supplied to the end
      user with special focus on minimizing the delay and jitter.
    ...
    \label{sec:uc:prep}
      ...
      \label{ssub:}
        ...
        \begin{itemize}
          \item high priority RTP streaming - UDP traffic with source ports 6970 and 6971, used by streaming clients in 3.3.3.0/24
                subnetwork,
          \item low priority Video On Demand - TCP traffic with source port 80, used by the VOD client,
          \item medium priority ordinary traffic.
        \end{itemize}

        Furthermore, the streaming clients inside the 3.3.3.0/24 network are assigned different priority values. The
        3.3.3.2 client is favoured and has high priority for RTP data, whereas 3.3.3.3 client is assigned low
        priority.

        \begin{figure}[H]
          \includegraphics[width=\textwidth]{img/test-case/topology-om.pdf}
    ...
    \label{sec:uc:operation}
      ...
      \label{ssub:}
        ...
        \begin{figure}[H]
          \caption{8Mbps bandwidth limitation set for all the clients}
        \end{figure}
