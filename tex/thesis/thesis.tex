...
\begin{document}
 ...
 resource reservation provided by most common OS induced further research in field of resource reservation and isolation. 
  ...
  \label{chap:bck}
	...
	Resource virtualization is in principle separation application from physical hardware on which it runs. It also allows dividing 
	single physical hardware to multiple virtualized logical resources, thereby running the software under virtualized resources
	lowers cost, improves efficiency, introduces isolation and better configurability. 



      \subsection{The need for virtualization}

	Together with increasing hardware evolution problems with its better utilization arised. Low fault tolerance, no isolation 
	and poor utilization of available resources implicated the demand of creating an approach overcoming this issues. 

	It was not until 1960s that virtualization was first used for better hardware utilization of some large, mainframe hardware.
	
	Today, over 50 years later when computers got more common than ever problems of rigidity and underutilization are still 
	actual. Nowadays companies like VMWare invest more and more funds in research as they foresee that virtualization will 
	definately become even more popular and common in the future. \cite{virtualization}
      ...
      \subsection{Full virtualization}
        ...
        Full virtualization is a technological approach to virtualization also know as emulation of the underlying raw hardware (native
  ...
  \label{chap:impl}
    ...
    \label{sec:impl:env}
      ...
      \label{sec:impl:low}
        ...
        written and Java ProcessBuilder invoked. Created jims-crossbow module containes shared library allowing \gls{acr:crud}
        operations which subsequently are being accessed by \gls{acr:jna}, whereas most of \gls{acr:jims} low-level
        access was done through running shell scripts. Although running scripts by ProcessBuilder is faster in some
        cases accessing native libraries through libraries like \gls{acr:jna} or \gls{acr:jni} gives more
      ...
      \label{sec:impl:gui}
        ...
        application is based on invocating \gls{acr:jmx} Bean's operation and presenting received responses.
	...
	This chapter introduced to adopted approach to system implementation. Despite the fact that requested functionalities were 
	very demanding and quite complex final product meets all previously listed funtional requirements. Thanks to used technologies such as \gls{acr:jmx},
	system with fully independent business layer ( highly coupled MBeans ) were implemented. These technologies also helped with fulfiling defined
	non-functional requirements like: evolvability, extensibility, composability, etc. Definately one of the biggest advantage of this system 
	( apart from all delivered functionality provided by \gls{acr:cm4j} ) is reusability as three possible usage models exist. System
	may be managed from implemented \gls{acr:gui}, running jconsole or even from own developed application where developer
	can decide which part of \gls{acr:cm4j} he would like to use by just conforming to previously presented and  described interfaces.
